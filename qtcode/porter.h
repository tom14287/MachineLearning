...
void stemfile(struct stemmer * z, FILE * f, string filepath,string filename)
{	
...
void tostemm(FILE *f,string open_filepath,string open_filename,string save_filepath,string save_filename)
{
	int i;
	struct stemmer * z = create_stemmer();
	s = (char *)malloc(i_max + 1);
//	f = fopen("1.txt", "r");
	string open = open_filepath + open_filename;
	fopen_s(&f, open.c_str(), "r");
	if (f == NULL) { 
		printf("File not found\n");
		system("error");
	}
	stemfile(z, f,save_filepath,save_filename);
	fclose(f);
	free(s);
	free_stemmer(z);
}
